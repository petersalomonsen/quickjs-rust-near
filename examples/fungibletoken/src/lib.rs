...
mod tests {
    ...
    const OPERATOR_DEDUCTION_JS: &str =
        include_str!("../e2e/operator-deduction.js");

    fn deploy_operator_deduction(contract: &mut Contract) {
        set_current_account_id(bob());
        set_predecessor_account_id(bob());
        contract.post_javascript(OPERATOR_DEDUCTION_JS.to_string());
    }

    fn register(contract: &mut Contract, account: AccountId) {
        let prev = NearToken::from_yoctonear(0);
        set_predecessor_account_id(account.clone());
        set_attached_deposit(contract.storage_balance_bounds().min.into());
        contract.storage_deposit(Some(account), Some(true));
        set_attached_deposit(prev);
    }

    fn call(contract: &mut Contract, predecessor: AccountId, function: &str, input: &str) {
        set_predecessor_account_id(predecessor);
        set_attached_deposit(NearToken::from_yoctonear(0));
        set_input(input.into());
        contract.call_js_func(function.to_string());
    }

    #[test]
    fn test_operator_deduction_happy_path() {
        setup_test_env();
        let mut contract = Contract::new_default_meta(bob().into(), TOTAL_SUPPLY.into());
        deploy_operator_deduction(&mut contract);
        register(&mut contract, alice());
        register(&mut contract, carol());

        set_predecessor_account_id(bob());
        set_attached_deposit(NearToken::from_yoctonear(1));
        contract.ft_transfer(alice(), 10_000u128.into(), None);

        set_block_timestamp(1_000_000_000_000_000);
        call(
            &mut contract,
            alice(),
            "authorize_deduction",
            r#"{"operator_account":"carol.near","max_amount_per_day":"5000"}"#,
        );
        ...
        call(
            ...
            carol(),
            "deduct",
            r#"{"user":"alice.near","amount":"1500","description":"sync"}"#,
        );

        assert_eq!(contract.ft_balance_of(alice()).0, 10_000 - 1_500);
        assert_eq!(contract.ft_balance_of(carol()).0, 1_500);
    }

    #[test]
    #[should_panic(expected = "no authorisation")]
    fn test_deduct_unauthorised_operator_panics() {
        setup_test_env();
        let mut contract = Contract::new_default_meta(bob().into(), TOTAL_SUPPLY.into());
        deploy_operator_deduction(&mut contract);
        register(&mut contract, alice());
        ...
        call(
            ...
            carol(),
            "deduct",
            r#"{"user":"alice.near","amount":"1","description":"x"}"#,
        );
    }

    #[test]
    #[should_panic(expected = "daily cap exceeded")]
    fn test_deduct_exceeding_daily_cap_panics() {
        setup_test_env();
        let mut contract = Contract::new_default_meta(bob().into(), TOTAL_SUPPLY.into());
        deploy_operator_deduction(&mut contract);
        register(&mut contract, alice());
        ...
        call(
            ...
            alice(),
            "authorize_deduction",
            r#"{"operator_account":"carol.near","max_amount_per_day":"1000"}"#,
        );
        ...
        call(
            ...
            carol(),
            "deduct",
            r#"{"user":"alice.near","amount":"600","description":""}"#,
        );
        call(
            &mut contract,
            carol(),
            "deduct",
            r#"{"user":"alice.near","amount":"500","description":""}"#,
        );
    }

    #[test]
    fn test_deduct_resets_after_24h() {
        setup_test_env();
        let mut contract = Contract::new_default_meta(bob().into(), TOTAL_SUPPLY.into());
        deploy_operator_deduction(&mut contract);
        register(&mut contract, alice());
        ...
        call(
            ...
            carol(),
            "deduct",
            r#"{"user":"alice.near","amount":"800","description":""}"#,
        );
        // Day 1 (>24h): counter resets, full cap available again
        set_block_timestamp(1_000_000_000_000_000 + 25 * 60 * 60 * 1_000_000_000);
        call(
            &mut contract,
            carol(),
            "deduct",
            r#"{"user":"alice.near","amount":"900","description":""}"#,
        );

        assert_eq!(contract.ft_balance_of(alice()).0, 10_000 - 800 - 900);
        assert_eq!(contract.ft_balance_of(carol()).0, 800 + 900);
    }

    #[test]
    #[should_panic(expected = "no authorisation")]
    fn test_revoke_then_deduct_panics() {
        setup_test_env();
        let mut contract = Contract::new_default_meta(bob().into(), TOTAL_SUPPLY.into());
        deploy_operator_deduction(&mut contract);
        register(&mut contract, alice());
        ...
        call(
            ...
            alice(),
            "revoke_deduction",
            r#"{"operator_account":"carol.near"}"#,
        );
        call(
            &mut contract,
            carol(),
            "deduct",
            r#"{"user":"alice.near","amount":"1","description":""}"#,
        );
    }

    #[test]
    fn test_reauthorize_updates_cap_and_resets_counter() {
        setup_test_env();
        let mut contract = Contract::new_default_meta(bob().into(), TOTAL_SUPPLY.into());
        deploy_operator_deduction(&mut contract);
        register(&mut contract, alice());
        ...
        // Re-authorize with a higher cap; counter resets to 0
        call(
            &mut contract,
            alice(),
            "authorize_deduction",
            r#"{"operator_account":"carol.near","max_amount_per_day":"2000"}"#,
        );
        call(
            &mut contract,
            carol(),
            "deduct",
            r#"{"user":"alice.near","amount":"1500","description":""}"#,
        );

        assert_eq!(contract.ft_balance_of(carol()).0, 800 + 1500);
    }

    #[test]
    fn test_view_authorisation_and_spent() {
        setup_test_env();
        let mut contract = Contract::new_default_meta(bob().into(), TOTAL_SUPPLY.into());
        deploy_operator_deduction(&mut contract);
        register(&mut contract, alice());
        ...
        call(
            ...
            carol(),
            "deduct",
            r#"{"user":"alice.near","amount":"700","description":""}"#,
        );

        set_input(
            r#"{"user":"alice.near","operator_account":"carol.near"}"#.into(),
        );
        contract.view_js_func("view_spent_since_reset".to_string());
        assert_latest_return_value_string_eq("\"700\"".to_string());
    }
