...
describe("Fungible token contract", { only: true }, () => {
  ...
  before(async () => {
    ...
    unregistered_bob = await worker.rootAccount.createAccount(
      "unregistered-bob.test.near",
    );
  ...
  test("should run custom javascript transfer functions in contract with function access keys, and without attaching deposits", async () => {
    const nearConnection = await connect(connectionConfig);
    const accountId = contract.accountId;

    const account = await nearConnection.account(accountId);
    const javascript = (
      await readFile(new URL("aiconversation.js", import.meta.url))
    )
      .toString()
      .replace(
        "REPLACE_REFUND_SIGNATURE_PUBLIC_KEY",
        JSON.stringify(
          Array.from((await contract.getKey()).getPublicKey().data),
        ),
      );

    const real_conversation_id = `alice.test.near_${new Date().getTime()}`;
    const conversation_id = createHash("sha256")
      .update(Buffer.from(real_conversation_id, "utf8"))
      .digest("hex");
    ...
    const refund_message_hashed = createHash("sha256")
      ...
      .digest();
    const signature = (await contract.getKey()).sign(
  ...
  test("should buy tokens for NEAR", { only: true }, async () => {
    const nearConnection = await connect(connectionConfig);
    const accountId = contract.accountId;
    ...
    expect(
      await contract.view("storage_balance_of", {
        account_id: unregistered_bob.accountId,
      }),
    ).to.equal(null);
    ...
    await unregistered_bob.call(
      ...
      {
        attachedDeposit: 500_000_000_000_000_000_000_000n.toString(),
      },
    );
