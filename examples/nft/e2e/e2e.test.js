...
describe("NFT contract", () => {
  ...
  test("should run custom javascript (quickjs bytecode ) in contract", async () => {
    ...
    await account.functionCall({
      ...
      args: {
        key: "/index.html",
        valuebase64: await (
          await readFile("web4/dist/index.html")
        ).toString("base64"),
      },
    });
    ...
    const wasmresult = await account.viewFunction({
      contractId: accountId,
      methodName: "web4_get",
      args: { request: { path: "/musicwasms/fall.wasm" } },
    });
  ...
  test("should require owners signature to get content", async () => {
    ...
    expect(result.body).to.equal(
      Buffer.from("DEFAULT RESPONSE").toString("base64"),
    );
    ...
    const signature = await keyPair.sign(
      new TextEncoder().encode(messageToBeSigned),
    );
    ...
    expect(result.contentType).to.equal("text/plain; charset=UTF-8");
    expect(result.body).to.equal(
      Buffer.from("VALID SIGNATURE").toString("base64"),
    );
  ...
  test("should list NFT owners through web4", async () => {
    ...
    let result = await account.viewFunction({
      contractId: accountId,
      methodName: "web4_get",
      args: { request: { path: "/nftowners.json" } },
    });
    ...
    expect(
      nftOwners.find((t) => t.token_id == mintedTokenIds[0]).owner_id,
    ).to.equal(accountId);
    expect(
      nftOwners.find((t) => t.token_id == mintedTokenIds[1]).owner_id,
    ).to.equal(accountId);
    expect(
      nftOwners.find((t) => t.token_id == mintedTokenIds[2]).owner_id,
    ).to.equal(accountId);
  });
  ...
  test("should forbid mint", async () => {
    ...
    try {
      ...
      expect(e.kind.kind.FunctionCallError.ExecutionError).to.equal(
        "Smart contract panicked: mint is forbidden",
      );
    }
  });
  ...
  test("15 TGas should be sufficient for nft_transfer_payout", async () => {
    ...
    expect(
      JSON.parse(Buffer.from(payout.status.SuccessValue, "base64").toString())
        .payout[accountId],
    ).to.equal(BigInt(10_0000_00000_00000_00000_00000n).toString());
    const tokenAfterTransfer = await account.viewFunction({
      contractId: accountId,
  ...
  test("should mint and burn", async () => {
    ...
    const tokenAfterMint = await account.viewFunction({
      contractId: accountId,
      methodName: "nft_token",
      args: { token_id },
    });
    ...
    const tokenAfterBurn = await account.viewFunction({
      contractId: accountId,
      methodName: "nft_token",
      args: { token_id },
    });
