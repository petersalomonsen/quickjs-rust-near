...
const { AI_PROXY_BASEURL, RPC_URL, NETWORK_ID, FUNGIBLE_TOKEN_CONTRACT_ID} = process.env;
...
if (!FUNGIBLE_TOKEN_CONTRACT_ID) {
  console.error('Environment variable FUNGIBLE_TOKEN_CONTRACT_ID not set. Must be set to the fungible token contract id (e.g. arizcredits.near)');
  process.exit(1);
}
  ...
  plugins: [html({ minify: true }), terser(), {
    ...
    closeBundle: () => {
      const js = readFileSync('dist/main.js').toString()
        ...
        .replace(`localStorage.getItem("contractId")`, `"${FUNGIBLE_TOKEN_CONTRACT_ID}"`)
