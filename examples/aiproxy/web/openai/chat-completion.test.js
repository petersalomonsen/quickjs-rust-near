...
test('should extract tool calls from a stream', async () => {
  ...
  const result = await sendStreamingRequest({ proxyUrl: "https://aiproxy", messages: [], onError: (err) => { console.error(err) } });

  equal(JSON.stringify(result[0].tool_calls), JSON.stringify([
    {
