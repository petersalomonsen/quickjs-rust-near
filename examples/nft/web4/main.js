...
(async () => {
  if ("serviceWorker" in navigator) {
    ...
    while (registration.active.state !== "activated") {
      console.log(
        `Waiting for service worker to be activated. Current state is ${registration.active.state}`,
      );
    ...
    playerElement.addEventListener("timeupdate", (event) => {
      const percentage =
        (playerElement.currentTime * 100) / playerElement.duration;
      ...
      timeSpan.innerHTML = new Date(playerElement.currentTime * 1000)
        .toJSON()
        .substring(timeStringStart, timeStringStart + "mm:ss".length);
    });
    const setPlayIcon = () => {
    ...
    let pos = 0;
    messageArea.innerHTML = musicwasms
      .map((song) => {
        const jsondateminutesstart = "yyyy-MM-dd HH:".length;
        const timestring = new Date(pos * 1000)
          .toJSON()
          .substring(jsondateminutesstart, jsondateminutesstart + 5);
    ...
    const nftowners = await (
      await fetch(
        new URL("nftowners.json?t=" + new Date().getTime(), import.meta.url),
      )
    ).json();
    ...
    nftownerstablebody.innerHTML = nftowners
      .map(
        (o, ndx) => `<tr>
                <td>${o.token_id}</td>
    ...
    nftowners.forEach(async (o, ndx) => {
      ...
      if (profile[o.owner_id] && profile[o.owner_id].profile.image.ipfs_cid) {
        document.getElementById(o.owner_id + "_profileimage_" + ndx).src =
          `https://i.near.social/thumbnail/https://ipfs.near.social/ipfs/${profile[o.owner_id].profile.image.ipfs_cid}`;
      }
    });
  } else {
    document.documentElement.innerHTML =
      "This app requires that your browser supports serviceworkers";
  }
})();
