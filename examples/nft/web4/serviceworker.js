...
const musicwasms = [
  {
    ...
    path: "grooveisinthecode.wasm",
    durationSeconds: 154,
  },
  {
    name: "Noise and madness",
    path: "noiseandmadness.wasm",
    durationSeconds: 163,
  },
  ...
  {
    name: "WebAssembly summit 2020",
    path: "wasmsummit2020.wasm",
    durationSeconds: 154,
  },
  ...
  {
    name: "WebAssembly summit 2021",
    path: "wasmsummit2021.wasm",
    durationSeconds: 191,
  },
];
...
const wasmInstancesReceivedPromise = new Promise(async (resolve) => {
  const wasmbytes = await Promise.all(
    musicwasms.map((wasmInstanceData) =>
      fetch(`/musicwasms/${wasmInstanceData.path}`).then((r) =>
        r.arrayBuffer(),
      ),
    ),
  );
  for (let n = 0; n < musicwasms.length; n++) {
    ...
    const wasmInstance = (
      await WebAssembly.instantiate(wasmbytes[n], {
        environment: {
    ...
    wasmInstances.push({
      ...
      renderPos: startPos,
      allocatedSampleBuffer: wasmInstance.allocateSampleBuffer
        ? wasmInstance.allocateSampleBuffer(SAMPLE_FRAMES)
        : undefined,
    });
...
function nextChunk(wantedRangeStart, wantedRangeEnd) {
  const currentWasmInstanceIndex = wasmInstances.findIndex(
    (instanceData, ndx) =>
  ...
  const samplebuffer = instanceData.allocatedSampleBuffer
    ? instanceData.allocatedSampleBuffer
    : wasmInstance.samplebuffer;

  for (
    let n = 0;
    n < 200 && currentBytePos < endpos && currentBytePos < totalLength;
    n++
  ) {
    ...
    for (
      let bufferpos = 0;
      bufferpos < SAMPLE_FRAMES && currentBytePos < totalLength;
      bufferpos++
    ) {
  ...
  if (rangeEnd < rangeStart) {
    rangeEnd = endpos - 1;
    console.log(
      `had to send empty data for song ${currentWasmInstanceIndex}, range: ${rangeStart} - ${rangeEnd}, totally ${((rangeEnd - rangeStart) / ONE_SECOND).toFixed(3)} seconds`,
    );
  }
...
self.addEventListener("fetch", (event) =>
  event.respondWith(
    new Promise(async (resolve) => {
      if (
        event.request.url.indexOf(".wav") > -1 &&
        event.request.headers.has("range")
      ) {
        const range = event.request.headers
          .get("range")
          .match(/bytes=([0-9]+)-([0-9]*)/);
        ...
        let { rangeStart, rangeEnd } = nextChunk(
          wantedRangeStart,
          wantedRangeEnd,
        );
        for (
          let n = 0;
          n < 4 && wantedRangeEnd && rangeEnd < wantedRangeEnd;
          n++
        ) {
          rangeEnd = nextChunk(rangeEnd + 1, wantedRangeEnd).rangeEnd;
        }
        ...
        resolve(
          new Response(
            new Blob([JSON.stringify(musicwasms)], {
