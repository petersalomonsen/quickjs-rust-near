...
export async function handleNearAILoginCallback() {
    ...
    if (accountId) {
        const nearaiauthobject = JSON.parse(localStorage.getItem(NEAR_AI_AUTH_OBJECT_STORAGE_KEY));
        ...
        localStorage.setItem(NEAR_AI_AUTH_OBJECT_STORAGE_KEY, JSON.stringify(nearaiauthobject));
        location.hash = "";
    }
}
...
export async function nearAIlogin(wallet, message) {
  ...
  const nonceBuffer = Buffer.from(new TextEncoder().encode(nonce.padStart(32, "0")));
  ...
  const signedMessage = await wallet.signMessage({message, nonce: nonceBuffer, recipient, callbackUrl});
