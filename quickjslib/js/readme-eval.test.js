...
async function executeSnippet(code) {
  ...
  executable = executable.replace(/import\s*{\s*createQuickJS\s*}\s*from\s*["'].*?["'];?\s*/g, '');

  // Add quickjs initialization if needed
  if (executable.includes('quickjs.') && !executable.includes('createQuickJS')) {
...
for (const group of grouped) {
  test(`README Line ${group.lineNumber}: ${group.section}`, async () => {
    ...
    if (!result.success) {
      throw new Error(`Snippet failed: ${result.error}\n\nCode:\n${group.code}`);
    }
  });
}